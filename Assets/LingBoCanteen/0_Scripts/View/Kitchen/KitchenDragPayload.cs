...
namespace LingBoCanteen
{
    ...
    public enum KitchenDragItemKind
    {
        ...
        /// <summary>[已不使用] 锅本身不允许拖拽，保留该枚举值仅为避免外部引用报错。</summary>
        Pot = 2,
    ...
    /// <summary>
    /// 烹调区拖拽负载：涵盖碗/杯槽产出物、调料投放到锅具，以及糊锅食物拖到垃圾桶两种场景
    /// （锅本身不可拖拽）。复用同一个类是因为三者都只需要"一个 Id + 一张图标 + 落地/弹回回调"，用 <see cref="Kind"/> 区分
    /// 各工位是否愿意接收，避免食物被误判成食材投进另一口锅。
    /// </summary>
